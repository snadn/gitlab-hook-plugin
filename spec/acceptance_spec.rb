...
feature 'GitLab WebHook' do
  ...
  before(:all) do
    ...
    [ 'tagbuilder8x' , 'multiscm8x' , 'subdirjob8x' ].each do |jobdir|
  ...
  feature 'GitLab 8.x' do
   ...
   feature 'Merge request handling' do
    ...
    scenario 'Create project with merge request' do
      ...
      expect(@gitlab.last('testrepo8x')).to eq '/mr_comment/5 - [Jenkins CI result SUCCESS](http://localhost:8080/job/testrepo8x-mr-feature_branch/1/)'
    end
    ...
    scenario 'Builds a push to merged branch (master)' do
      ...
      expect(@gitlab.last('testrepo8x')).to eq '/mr_comment/5 - [Jenkins CI result SUCCESS](http://localhost:8080/job/testrepo8x-mr-feature_branch/2/)'
    end
